...
class GridSepatuWanita extends StatefulWidget {
  final int userId;
  const GridSepatuWanita({super.key, required this.userId,});
...
class _GridSepatuWanitaState extends State<GridSepatuWanita> {
  ...
  Future<void> getAllSepatuWanita() async{
    String urlSepatuWanita = "https://backend-mobile.yoliza277.online/gridsepatuwanita.php"; // "http://localhost/server_shop_yoliza/gridsepatuwanita.php";
    try {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text("Women's Shoe Product", style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Colors.white),),
        centerTitle: true,
            ...
            return GestureDetector(
              onTap: () {
                Navigator.pushReplacement(
                  context,
                  MaterialPageRoute(
                    builder: (context) => DetilSepatuWanita(item: item, userId: widget.userId,),
                  ),
                );
              },
              child: Card(
...
class DetilSepatuWanita extends StatelessWidget {
  ...
  const DetilSepatuWanita({super.key, required this.item, required this.userId,});
      ...
      appBar: AppBar(
        leading: IconButton(onPressed: () {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (context) => GridSepatuWanita(userId: userId),
            ),
          );
        },
