...
class GridBajuPria extends StatefulWidget {
  final int userId;
  const GridBajuPria({super.key, required this.userId,});
...
class _GridBajuPriaState extends State<GridBajuPria> {
  ...
  Future<void> getAllBajuPria() async{
    String urlBajuPria = "https://backend-mobile.yoliza277.online/gridbajupria.php"; // "http://localhost/server_shop_yoliza/gridbajupria.php";
    try {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text("Men's Shirt Product", style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Colors.white),),
        centerTitle: true,
            ...
            return GestureDetector(
              onTap: () {
                Navigator.pushReplacement(
                  context,
                  MaterialPageRoute(
                    builder: (context) => DetilBajuPria(item: item, userId: widget.userId,),
                  ),
                );
              },
              child: Card(
...
class DetilBajuPria extends StatelessWidget {
  ...
  const DetilBajuPria({super.key, required this.item, required this.userId});
      ...
      appBar: AppBar(
        leading: IconButton(onPressed: () {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (context) => GridBajuPria(userId: userId),
            ),
          );
        },
