...
class GridElectronic extends StatefulWidget {
  final int userId;
  const GridElectronic({super.key, required this.userId,});
...
class _GridElectronicState extends State<GridElectronic> {
  ...
  Future<void> getAllElectronic() async{
    String urlElectronic = "https://backend-mobile.yoliza277.online/gridelectronic.php"; // "http://localhost/server_shop_yoliza/gridelectronic.php";
    try {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text("Electronic Product", style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Colors.white),),
        centerTitle: true,
            ...
            return GestureDetector(
              onTap: () {
                Navigator.pushReplacement(
                  context,
                  MaterialPageRoute(
                    builder: (context) => DetilElektronic(item: item, userId: widget.userId,),
                  ),
                );
              },
              child: Card(
...
class DetilElektronic extends StatelessWidget {

  final dynamic item;
  final int userId;

  const DetilElektronic({super.key, required this.item, required this.userId});
      ...
      appBar: AppBar(
        leading: IconButton(onPressed: () {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (context) => GridElectronic(userId: userId),
            ),
          );
        },
