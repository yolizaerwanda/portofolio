...
export async function GET(req: Request) {
  try {
    ...
    const namaSales = searchParams.get("nama_sales");

    if (!namaSales) {
      return NextResponse.json(
        { error: "Parameter nama_sales wajib." },
        { status: 400 }
      );
    }

    // Ambil data retur berdasarkan nama_sales
    const [rows] = await db.query(
      `SELECT * FROM retur WHERE nama_sales = ? ORDER BY tgl_terima DESC`,
      [namaSales]
    );

    if (!(rows as any[]).length) {
      return NextResponse.json(
        { error: "Tidak ada data untuk sales ini." },
        { status: 404 }
      );
    }
    ...
    // Hitung total retur
    const totalRetur = (rows as any[]).reduce(
      (acc, r) => acc + Number(r.total_retur),
      0
    );
    ...
    // Judul laporan
    page.drawText("Laporan Retur Sales", {
      x: width / 2 - 140,
      y,
      size: 20,
      font: fontBold,
    ...
    page.drawText(`Nama Sales: ${namaSales}`, {
      x: 65,
      y,
      size: 12,
      font,
    ...
    page.drawText(`Jumlah Total Retur: ${rupiah(totalRetur)}`, {
      x: tableMarginX,
      y,
      size: 12,
      font,
    ...
    return new Response(Buffer.from(pdfBytes), {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="laporan-${namaSales}.pdf"`,
      },
    });
  } catch (err) {
    console.error(err);
